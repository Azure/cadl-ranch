...
class ServerTestsGenerator {
  ...
  private getConfigObj() {
    ...
    if (["head", "get", "delete"].includes(this.mockApiDefinition.method) && this.mockApiDefinition.request.body) {
      config = {
        ...config,
  ...
  public async executeScenario() {
    ...
    if (this.mockApiDefinition.response.body) {
      if (this.mockApiDefinition.response.body.contentType === "application/xml") {
        if (JSON.stringify(this.mockApiDefinition.response.body.rawContent) !== JSON.stringify(response.data)) {
        ...
        if (
          ...
          if (response.data.content !== this.mockApiDefinition.response.body.rawContent.toString("base64")) {
            throw new Error(`Response data mismatch for ${this.name} endpoint`);
          }
        } else {
          if (
            uint8ArrayToString(response.data, "utf-8") !== this.mockApiDefinition.response.body.rawContent.toString()
          ) {
            throw new Error(`Response data mismatch for ${this.name} endpoint`);
          }
        }
        ...
        if (this.mockApiDefinition.response.body.rawContent !== responseData.replace(this.serverBasePath, "")) {
          logger.error(`Response data mismatch for ${this.name} endpoint`);
          logger.error(`Expected: ${this.mockApiDefinition.response.body} - Actual: ${response.data}`);
          throw new Error(`Response data mismatch for ${this.name} endpoint`);
        }
    ...
    if (this.mockApiDefinition.response.headers) {
      for (const key in this.mockApiDefinition.response.headers) {
        if (this.mockApiDefinition.response.headers[key] !== response.headers[key].replace(this.serverBasePath, "")) {
