...
describe("authentication/http/custom endpoint", () => {
  ...
  it("should return 204 when the apiKey is valid", async () => {
    const endPoint = `${serverBasePath}/authentication/http/custom/valid`;
  ...
  it("should return 403 when the apiKey is invalid", async () => {
    const endPoint = `${serverBasePath}/authentication/http/custom/invalid`;
    const response = await makeServiceCall(SERVICE_CALL_TYPE.get, {
      endPoint,
      options: {
        config: {
